...
function getAll(req, res, next) {
  UserService.find({}, { password: false })
    .then(users => {
      res.send({ data: users });
    })
    .catch(err => {
...
function removeUser(req, res, next) {
  UserService.findById(req.params._id)
    .then(user => {
...
function authenticate(req, res, next) {
  if (req.body.username && req.body.password) {
    ...
    UserService.findOne(conditions)
      .then(user => {
        if (user) {
    ...
    .then(user => {
      ...
      user
        ...
        .catch(ex => {
          res.status(500);
          res.send({
            data: "service not available. Database error save token"
          });
        });
    })
    .catch(ex => {
      res.status(500);
      res.send({
        data: "service not available. Database error could not find user"
      });
    });
}
...
function checkEmailVerification(req, res, next) {
  if (!req.params.token) {
    res.status(400);
    res.send({
      data: [{ param: "token", msg: "verification token value is missing" }]
    });
  }
